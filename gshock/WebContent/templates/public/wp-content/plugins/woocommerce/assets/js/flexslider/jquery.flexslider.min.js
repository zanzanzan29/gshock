! function(u) {
    ...
    u.flexslider = function(p, e) {
        ...
        u.data(p, "flexslider", m), c = {
            ...
            resize: function() {
                !m.animating && m.is(":visible") && (S || m.doMath(), x ? c.smoothHeight() : S ? (m.slides.width(m.computedW), m.update(m.pagingCount), m.setProps()) : g ? (m.viewport.height(m.h), m.setProps(m.h, "setTotal")) : (m.setProps(m.computedW, "setTotal"), m.newSlides.width(m.computedW), m.vars.smoothHeight && c.smoothHeight()))
            },
